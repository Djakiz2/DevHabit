    ...
    ILogger<ProcessEntryImportJob> logger) : IJob
{
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        EntryImportJob? importJob = await dbContext.EntryImportJobs
            .FirstOrDefaultAsync(j => j.Id == importJobId, cancellationToken:  context.CancellationToken);
        ...
        try
        {
            ...
            var records = new List<CsvEntryRecord>();

            await foreach (CsvEntryRecord record in csv.GetRecordsAsync<CsvEntryRecord>(context.CancellationToken))
            {
                records.Add(record);
            }
            ...
            foreach (CsvEntryRecord record in records)
            {
                try
                {
                    ...
                    Habit? habit = await dbContext.Habits
                        .FirstOrDefaultAsync(h => h.Id == record.HabitId && h.UserId == importJob.UserId,context.CancellationToken);
