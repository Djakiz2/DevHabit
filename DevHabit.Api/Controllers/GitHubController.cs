    ...
    LinkService linkService) : ControllerBase
{
    ...
    public async Task<IActionResult> RevokeAcessToken()
    {
        string? userId = await userContext.GetUserIdAsync();
    ...
    public async Task<ActionResult<GitHubUserProfileDto>> GetUserProfile([FromHeader] AcceptHeaderDto acceptHeader)
    {
        ...
        if (acceptHeader.IncludeLinks)
        {
            ...
            [
                ...
                linkService.Create(nameof(RevokeAcessToken), "revoke-token",
                HttpMethods.Delete)
            ];
        }
    ...
    public async Task<ActionResult<IReadOnlyList<GitHubEventDto>>> GetUserEvents()
    {
        ...
        IReadOnlyList<GitHubEventDto>? events = await gitHubService.GetUserEventsAsync(profile.Login, accessToken);
